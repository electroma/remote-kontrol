...
class ClosureCommandGenerator @JvmOverloads constructor(private val classLoader: ClassLoader = Thread.currentThread().contextClassLoader) : CommandGenerator<RawClosureCommand, ClosureCommand> {
    ...
    override fun generate(rawClosureCommand: RawClosureCommand): ClosureCommand {
        ...
        if (!used.isEmpty()) {
            for (usedClosure in used) {
                supports.add(getClassBytes(usedClosure.javaClass))
                supports.addAll(getSupportingClassesBytes(usedClosure.javaClass))
            }
        }*/
     ...
     */
    protected fun getSupportingClassesBytes(closureClass: Class<out FunctionImpl>): List<ByteArray> {
        try {
     ...
     */
    protected fun getClassBytes(closureClass: Class<out FunctionImpl>): ByteArray {

...
*/
        ...
        remote = RemoteControl(transport!!, clientClassLoader)
    }
    ...
    fun testCanSpecifyToUseNullIfReturnWasUnserializable() {
        remote = RemoteControl(transport!!, UnserializableResultStrategy.NULL, clientClassLoader)
        assertNull(remote!!({ System.out }))
    }

    @Test
    fun testCanSpecifyToUseStringRepresentationIfReturnWasUnserializable() {
        remote = RemoteControl(transport!!, UnserializableResultStrategy.STRING, clientClassLoader)
    ...
    fun testPassingUsedClosuresFromFields() {
        // TODO: need to pass classes instead of instances!
        assertEquals(2, remote!!.exec(arrayOf({ globalContextClosure() + 1 })))
    }
